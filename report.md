...
1. Add automated unit tests that assert `pricing`, `shipping`, and `risk_assessment` exist for every order processed by the baseline.
2. Run the harness on a larger, more diverse fixture set and bootstrap confidence intervals.
3. If you want to check summary quality, add an LLM-based semantic judge (cost varies by model).

---
This report was produced by the project harness; raw results are in `results.json` and the evaluation design is in `DESIGN.md`.
# Evaluation Report — Stakeholder Summary
